...
router.post("/add", async (req, res) => {
  ...
  try {
    // Inserts directly into your gold tracking table
    const query = `
      INSERT INTO gold_prices (karat, price, date) 
      VALUES ($1, $2, $3) 
    ...
    return res.status(201).json({
      success: true,
      message: "Gold price details added successfully",
      data: result.rows[0]
    });
...
// ---------------------------------------------------------
// 4. DELETE: Delete Gold Record by ID
...
router.delete("/delete/:id", async (req, res) => {
  ...
  try {
    const query = "DELETE FROM gold_prices WHERE id = $1 RETURNING id, karat, price, date;";
